...
class TestTestMixin:
    ...
    def test_create_safety_test_sync_different_policies(self, aymara_client, safety_test_data, test_policy):
    ...
    def test_create_multiturn_test_sync(self, aymara_client: AymaraAI, safety_test_data):
        response = aymara_client.create_test(**safety_test_data)
    ...
    async def test_create_jailbreak_test_async(self, aymara_client, jailbreak_test_data):
        response = await aymara_client.create_jailbreak_test_async(**jailbreak_test_data)
        assert isinstance(response, JailbreakTestResponse)
        assert response.test_status == Status.COMPLETED
    ...
    async def test_get_test_async(self, aymara_client, jailbreak_test_data):
        created_test = await aymara_client.create_jailbreak_test_async(**jailbreak_test_data)
    ...
    def test_list_tests_sync(self, aymara_client, safety_test_data, jailbreak_test_data):
    ...
    async def test_list_tests_async(self, aymara_client, safety_test_data, jailbreak_test_data):
    ...
    def test_list_tests_as_df_sync(self, aymara_client, safety_test_data, jailbreak_test_data):
        aymara_client.create_safety_test(**safety_test_data)
        aymara_client.create_jailbreak_test(**jailbreak_test_data)
        df = aymara_client.list_tests().to_df()
        assert isinstance(df, pd.DataFrame)
        assert len(df) >= 2
        assert all(col in df.columns for col in ["test_uuid", "test_name", "test_status", "failure_reason"])

    async def test_list_tests_as_df_async(self, aymara_client, safety_test_data, jailbreak_test_data):
        await aymara_client.create_safety_test_async(**safety_test_data)
        await aymara_client.create_jailbreak_test_async(**jailbreak_test_data)
    ...
    def test_create_safety_test_invalid_inputs(self, aymara_client, safety_test_data, invalid_input):
    ...
    def test_create_jailbreak_test_invalid_inputs(self, aymara_client, jailbreak_test_data, invalid_input):
    ...
    def test_create_safety_test_timeout(self, aymara_client, safety_test_data):
        response = aymara_client.create_safety_test(**safety_test_data, max_wait_time_secs=0)
        assert response.test_status == Status.FAILED
        assert response.failure_reason == "Test creation timed out"

    async def test_create_jailbreak_test_async_timeout(self, aymara_client, jailbreak_test_data):
        response = await aymara_client.create_jailbreak_test_async(**jailbreak_test_data, max_wait_time_secs=0)
        assert response.test_status == Status.FAILED
        assert response.failure_reason == "Test creation timed out"
    ...
    def test_create_multiple_safety_tests(self, aymara_client, safety_test_data):
        responses = [aymara_client.create_safety_test(**safety_test_data) for _ in range(3)]
    ...
    async def test_create_multiple_jailbreak_tests_async(self, aymara_client, jailbreak_test_data):
        responses = await asyncio.gather(
            *[aymara_client.create_jailbreak_test_async(**jailbreak_test_data) for _ in range(3)]
        )
        assert all(isinstance(response, JailbreakTestResponse) for response in responses)
        assert all(response.test_status == Status.COMPLETED for response in responses)
    ...
    async def test_delete_jailbreak_test_async(self, aymara_client: AymaraAI, jailbreak_test_data):
        created_test = await aymara_client.create_jailbreak_test_async(**jailbreak_test_data)
        assert created_test.test_status == Status.COMPLETED
    ...
    async def test_create_image_safety_test_async(self, aymara_client, image_safety_test_data):
        response = await aymara_client.create_image_safety_test_async(**image_safety_test_data)
        assert isinstance(response, SafetyTestResponse)
        assert response.test_status == Status.COMPLETED
        assert len(response.questions) == image_safety_test_data["num_test_questions"]
    ...
    def test_create_image_safety_test_timeout(self, aymara_client, image_safety_test_data):
        response = aymara_client.create_image_safety_test(**image_safety_test_data, max_wait_time_secs=0)
        assert response.test_status == Status.FAILED
        assert response.failure_reason == "Test creation timed out"

    async def test_create_image_safety_test_async_timeout(self, aymara_client, image_safety_test_data):
        response = await aymara_client.create_image_safety_test_async(**image_safety_test_data, max_wait_time_secs=0)
        assert response.test_status == Status.FAILED
        assert response.failure_reason == "Test creation timed out"
    ...
    def test_create_image_safety_test_invalid_inputs(self, aymara_client, image_safety_test_data, invalid_input):
    ...
    class TestFreeUserRestrictions:
        ...
        def test_free_user_cannot_create_safety_test(self, free_aymara_client, safety_test_data):
            with pytest.raises(ValidationError):
                free_aymara_client.create_safety_test(**safety_test_data)

        def test_free_user_cannot_create_jailbreak_test(self, free_aymara_client, jailbreak_test_data):
            with pytest.raises(ValidationError):
                free_aymara_client.create_jailbreak_test(**jailbreak_test_data)

        async def test_free_user_cannot_create_safety_test_async(self, free_aymara_client, safety_test_data):
            with pytest.raises(ValidationError):
                await free_aymara_client.create_safety_test_async(**safety_test_data)

        async def test_free_user_cannot_create_jailbreak_test_async(self, free_aymara_client, jailbreak_test_data):
            with pytest.raises(ValidationError):
                await free_aymara_client.create_jailbreak_test_async(**jailbreak_test_data)
        ...
        async def test_free_user_list_tests_async_shows_default_tests(self, free_aymara_client):
            tests_list = await free_aymara_client.list_tests_async()
            assert isinstance(tests_list, ListTestResponse)
            assert len(tests_list) == self.NUM_DEFAULT_TESTS

        def test_free_user_list_tests_as_df_shows_default_tests(self, free_aymara_client):
            df = free_aymara_client.list_tests().to_df()
            assert isinstance(df, pd.DataFrame)
            assert len(df) == self.NUM_DEFAULT_TESTS

        async def test_free_user_list_tests_as_df_async_shows_default_tests(self, free_aymara_client):
            df = (await free_aymara_client.list_tests_async()).to_df()
            assert isinstance(df, pd.DataFrame)
            assert len(df) == self.NUM_DEFAULT_TESTS

        def test_free_user_cannot_create_image_safety_test(self, free_aymara_client, image_safety_test_data):
            with pytest.raises(ValidationError):
        ...
        async def test_free_user_cannot_create_image_safety_test_async(
            ...
            with pytest.raises(ValidationError):
                await free_aymara_client.create_image_safety_test_async(**image_safety_test_data)
    ...
    def test_invalid_additional_instructions(self, aymara_client, safety_test_data, additional_instructions):
        safety_test_data["additional_instructions"] = additional_instructions
        with pytest.raises(ValueError):
            aymara_client.create_safety_test(**safety_test_data)

    def test_create_safety_test_with_examples(self, aymara_client, safety_test_data, example_data):
    ...
    async def test_create_safety_test_async_with_examples(self, aymara_client, safety_test_data, example_data):
        safety_test_data.update(example_data)
        response = await aymara_client.create_safety_test_async(**safety_test_data)
        assert isinstance(response, SafetyTestResponse)
        assert response.test_status == Status.COMPLETED
    ...
    def test_create_jailbreak_test_with_examples(self, aymara_client, jailbreak_test_data, example_data):
    ...
    async def test_create_jailbreak_test_async_with_examples(self, aymara_client, jailbreak_test_data, example_data):
        jailbreak_test_data.update(example_data)
        response = await aymara_client.create_jailbreak_test_async(**jailbreak_test_data)
        assert isinstance(response, JailbreakTestResponse)
        assert response.test_status == Status.COMPLETED
    ...
    def test_create_image_safety_test_with_examples(self, aymara_client, image_safety_test_data, example_data):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            {
                "good_examples": [
                    GoodExample(question_text=f"Question {i}", explanation=f"Explanation {i}")
                    for i in range(MAX_EXAMPLES_LENGTH + 1)
                ]
            },
        ],
    )
    def test_create_safety_test_invalid_examples(self, aymara_client, safety_test_data, invalid_examples):
    ...
    def test_create_safety_test_with_policy_name(self, aymara_client, safety_test_data, safety_policy_data):
    ...
    async def test_create_safety_test_async_with_policy_name(self, aymara_client, safety_test_data, safety_policy_data):
        safety_test_data["test_policy"] = safety_policy_data.aymara_policy_name
        response = await aymara_client.create_safety_test_async(**safety_test_data)
        assert isinstance(response, SafetyTestResponse)
        assert response.test_status == Status.COMPLETED
    ...
    def test_create_image_safety_test_with_policy_name(
        ...
        image_safety_test_data["test_policy"] = image_safety_policy_data.aymara_policy_name
        response = aymara_client.create_image_safety_test(**image_safety_test_data)
        assert isinstance(response, SafetyTestResponse)
        assert response.test_status == Status.COMPLETED
    ...
    def test_create_safety_test_with_invalid_policy_name(self, aymara_client, safety_test_data):
        safety_test_data["test_policy"] = f"{AYMARA_TEST_POLICY_PREFIX}invalid_policy_name"
        with pytest.raises(ValidationError):
            aymara_client.create_safety_test(**safety_test_data)

    async def test_create_safety_test_async_with_invalid_policy_name(self, aymara_client, safety_test_data):
        safety_test_data["test_policy"] = f"{AYMARA_TEST_POLICY_PREFIX}invalid_policy_name"
        with pytest.raises(ValidationError):
            await aymara_client.create_safety_test_async(**safety_test_data)

    def test_create_image_safety_test_with_invalid_policy_name(self, aymara_client, image_safety_test_data):
        image_safety_test_data["test_policy"] = f"{AYMARA_TEST_POLICY_PREFIX}invalid_policy_name"
        with pytest.raises(ValidationError):
            aymara_client.create_image_safety_test(**image_safety_test_data)

    async def test_create_image_safety_test_async_with_invalid_policy_name(self, aymara_client, image_safety_test_data):
        image_safety_test_data["test_policy"] = f"{AYMARA_TEST_POLICY_PREFIX}invalid_policy_name"
        with pytest.raises(ValidationError):
    ...
    def test_create_accuracy_test_with_examples(self, aymara_client, accuracy_test_data):
        response = aymara_client.create_accuracy_test(**accuracy_test_data)
        assert isinstance(response, AccuracyTestResponse)
        assert response.test_status == Status.COMPLETED
    ...
    def test_create_accuracy_test_create_test_fn(self, aymara_client, accuracy_test_data):
        accuracy_test_data["test_type"] = TestType.ACCURACY
        response = aymara_client.create_test(**accuracy_test_data)
        assert isinstance(response, AccuracyTestResponse)
        assert response.test_status == Status.COMPLETED
    ...
    async def test_create_accuracy_test_async_with_examples(self, aymara_client, accuracy_test_data):
        response = await aymara_client.create_accuracy_test_async(**accuracy_test_data)
        assert isinstance(response, AccuracyTestResponse)
        assert response.test_status == Status.COMPLETED
    ...
    async def test_create_accuracy_test_async_timeout(self, aymara_client, accuracy_test_data):
        response = await aymara_client.create_accuracy_test_async(**accuracy_test_data, max_wait_time_secs=0)
        assert response.test_status == Status.FAILED
        assert response.failure_reason == "Test creation timed out"
    ...
    def test_create_accuracy_test_invalid_inputs(self, aymara_client, accuracy_test_data, invalid_input):
        invalid_data = {**accuracy_test_data, **invalid_input}
        with pytest.raises(ValueError):
            aymara_client.create_accuracy_test(**invalid_data)

    def test_free_user_cannot_create_accuracy_test(self, free_aymara_client, accuracy_test_data):
        with pytest.raises(ValidationError):
            free_aymara_client.create_accuracy_test(**accuracy_test_data)

    async def test_free_user_cannot_create_accuracy_test_async(self, free_aymara_client, accuracy_test_data):
        with pytest.raises(ValidationError):
