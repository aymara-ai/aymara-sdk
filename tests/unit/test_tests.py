...
def test_create_safety_test(aymara_client):
    with patch("aymara_ai.core.tests.create_test.sync_detailed") as mock_create_test, patch(
        "aymara_ai.core.tests.get_test.sync_detailed"
    ) as mock_get_test, patch("aymara_ai.core.tests.get_test_questions.sync_detailed") as mock_get_questions:
        ...
        result = aymara_client.create_safety_test("Test 1", "Student description", "Don't allow any unsafe answers")
...
async def test_create_jailbreak_test_async(aymara_client):
    with patch("aymara_ai.core.tests.create_test.asyncio_detailed") as mock_create_test, patch(
        "aymara_ai.core.tests.get_test.asyncio_detailed"
    ) as mock_get_test, patch("aymara_ai.core.tests.get_test_questions.asyncio_detailed") as mock_get_questions:
        mock_create_test.return_value.parsed = models.TestOutSchema(
            test_uuid="test123",
            test_name="Test 1",
            test_status=models.TestStatus.RECORD_CREATED,
...
def test_create_test_validation(aymara_client):
    with pytest.raises(ValueError, match="test_name must be between"):
        aymara_client.create_safety_test("A" * 256, "Student description", "Don't allow any unsafe answers")
    ...
    long_instructions = "B" * int(DEFAULT_MAX_TOKENS / DEFAULT_CHAR_TO_TOKEN_MULTIPLIER / 2)
    with pytest.raises(ValueError, match="tokens in total but they should be less than"):
        aymara_client.create_safety_test(
            "Test 1",
            long_desc,
            "Don't allow any unsafe answers",
...
async def test_get_test_async(aymara_client):
    with patch("aymara_ai.core.tests.get_test.asyncio_detailed") as mock_get_test, patch(
...
def test_validate_test_inputs_valid(aymara_client):
    aymara_client._validate_test_inputs(
        ...
        good_examples=[GoodExample(question_text="Question 1", explanation="Explanation 1")],
        bad_examples=[BadExample(question_text="Question 2", explanation="Explanation 2")],
    )
...
def test_validate_test_inputs_excessive_tokens(aymara_client):
    long_text_length = int(DEFAULT_MAX_TOKENS // (2 * DEFAULT_CHAR_TO_TOKEN_MULTIPLIER) + 1)
...
def test_create_and_wait_for_test_impl_sync_success(aymara_client):
    ...
    ), patch("aymara_ai.core.tests.get_test_questions.sync_detailed", mock_get_questions):
...
async def test_create_and_wait_for_test_impl_async_success(aymara_client):
    ...
    ), patch("aymara_ai.core.tests.get_test_questions.asyncio_detailed", mock_get_questions):
...
def test_create_and_wait_for_test_impl_timeout_sync(aymara_client):
    ...
    with patch("aymara_ai.core.tests.create_test.sync_detailed", mock_create), patch(
        "aymara_ai.core.tests.get_test.sync_detailed", mock_get
    ), patch("time.time", side_effect=mock_time), patch("time.sleep", return_value=None):
        result = aymara_client._create_and_wait_for_test_impl_sync(test_data, 60)
...
async def test_create_and_wait_for_test_impl_timeout_async(aymara_client):
    ...
    with patch("aymara_ai.core.tests.create_test.asyncio_detailed", mock_create), patch(
        "aymara_ai.core.tests.get_test.asyncio_detailed", mock_get
    ), patch("aymara_ai.core.tests.get_test_questions.asyncio_detailed", mock_get_questions), patch(
...
def test_get_all_questions_single_page_sync(aymara_client):
    ...
    with patch("aymara_ai.core.tests.get_test_questions.sync_detailed", mock_get_questions):
...
async def test_get_all_questions_single_page_async(aymara_client):
    ...
    with patch("aymara_ai.core.tests.get_test_questions.asyncio_detailed", mock_get_questions):
...
def test_logger_progress_bar(aymara_client):
    ...
    with patch("aymara_ai.core.tests.create_test.sync_detailed") as mock_create_test, patch(
        ...
        mock_logger.progress_bar.assert_called_once_with("Test 1", "test123", Status.PENDING)
...
def test_status_handling(aymara_client, test_status, expected_status, test_type):
    with patch("aymara_ai.core.tests.get_test.sync_detailed") as mock_get_test, patch(
        ...
        mock_get_test.return_value.parsed = models.TestOutSchema(
            ...
            updated_at=datetime.now(),
            test_policy="Don't allow any unsafe answers"
            if test_type == models.TestType.SAFETY or test_type == models.TestType.IMAGE_SAFETY
            else None,
            test_system_prompt="You are a helpful assistant" if test_type == models.TestType.JAILBREAK else None,
        ...
        if test_status == models.TestStatus.FINISHED:
            mock_get_test_questions.assert_called_once_with(client=aymara_client.client, test_uuid="test123", offset=0)
        else:
...
def test_delete_test(aymara_client):
    with patch("aymara_ai.core.tests.delete_test.sync_detailed") as mock_delete_test:
        ...
        mock_delete_test.assert_called_once_with(client=aymara_client.client, test_uuid="test123")


@pytest.mark.asyncio
async def test_delete_test_async(aymara_client):
    with patch("aymara_ai.core.tests.delete_test.asyncio_detailed") as mock_delete_test_async:
        mock_delete_test_async.return_value.status_code = 204  # No Content

        await aymara_client.delete_test_async("test123")

        mock_delete_test_async.assert_called_once_with(client=aymara_client.client, test_uuid="test123")
...
def test_create_accuracy_test(aymara_client):
    with patch("aymara_ai.core.tests.create_test.sync_detailed") as mock_create_test, patch(
        ...
        result = aymara_client.create_accuracy_test("Test 1", "Student description", "Test knowledge base content")
...
def test_create_accuracy_test_validation(aymara_client):
    with pytest.raises(ValueError, match="test_name must be between"):
        aymara_client.create_accuracy_test("A" * 256, "Student description", "Test knowledge base content")
    ...
    long_desc = "A" * int(DEFAULT_MAX_TOKENS / DEFAULT_CHAR_TO_TOKEN_MULTIPLIER / 2)
    long_knowledge_base = "B" * int(DEFAULT_MAX_TOKENS / DEFAULT_CHAR_TO_TOKEN_MULTIPLIER / 2)
    with pytest.raises(ValueError, match="tokens in total but they should be less than"):
        aymara_client.create_accuracy_test(
            "Test 1",
            long_desc,
