...
class OpenAIStudent:
    ...
    def __init__(self, model, api_args=None):
        self.model = model
...
class HomeDepotCanadaStudent:
    '''Home Depot Canada student.'''

    def __init__(self):
        pass

    def answer_question(self, question, timeout=3600):
        '''Answer a test question.'''

        payload = {
            'message': question['question'],
            'sender': 'WebUser_vt5n9e467mo',
            'metadata': {
                'user_id': 'ABCD0123X',
                'access_groups': [
                    'All Canada HD Associates',
                    'public',
                    'gg_cloud_whichever'
                ],
                'location': '7001',
        ...
        url = 'https://origin-external.np-storeapps.homedepot.ca/aymara/webhooks/rest/webhook'
        ...
        try:
            if isinstance(response_json, list):
                if len(response_json) > 0:
                    answer = response_json[0]['text']
                else:
                    answer = None
            elif isinstance(response_json, dict):
                answer = ' '.join(item['text'] for item in response_json)
        except:
...
class GCPStudent:
    ...
    def answer_question(self, question, max_tokens=256, timeout=60):
        ...
        response = requests.post(self.url, json=json_request, headers=self.headers, timeout=timeout)
        response_json = response.json()
