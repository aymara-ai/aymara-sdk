...
from aymara_sdk.types import CreateTestResponse, GetTestResponse, Status
...
from aymara_sdk.utils.errors import TestCreationError
...
class TestMixin(AymaraAIProtocol):
    ...
    def _create_test(
        ...
        is_async: bool,
    ) -> Union[CreateTestResponse, Coroutine[CreateTestResponse, None, None]]:
        ...
        if is_async:
            return self._create_and_wait_for_test_async(test_data)
        else:
            return self._create_and_wait_for_test_sync(test_data)
    ...
    def _create_and_wait_for_test_sync(
    ...
    ) -> Union[GetTestResponse, Coroutine[GetTestResponse, None, None]]:
        if is_async:
            return self._get_test_async_impl(test_uuid)
        else:
            return self._get_test_sync_impl(test_uuid)

    def _get_test_sync_impl(self, test_uuid: str) -> GetTestResponse:
    ...
    async def _get_test_async_impl(self, test_uuid: str) -> GetTestResponse:
        test_response = await core_api_get_test.asyncio(
            client=self.client, test_uuid=test_uuid
        )
        questions = None
    ...
    def list_tests(self) -> List[GetTestResponse]:
        ...
        """
        return self._list_tests(is_async=False)

    async def list_tests_async(self) -> List[GetTestResponse]:
        """
        List all tests asynchronously.
        """
        return await self._list_tests(is_async=True)

    def _list_tests(
        self, is_async: bool
    ) -> Union[List[GetTestResponse], Coroutine[List[GetTestResponse], None, None]]:
        if is_async:
            return self._list_tests_async_impl()
        else:
            return self._list_tests_sync_impl()

    def _list_tests_sync_impl(self) -> List[GetTestResponse]:
    ...
    async def _list_tests_async_impl(self) -> List[GetTestResponse]:
