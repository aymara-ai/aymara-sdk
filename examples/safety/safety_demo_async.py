...
from aymara_sdk.types import Status, StudentAnswer
...
async def main():
    ...
    async with AymaraAI(base_url="http://localhost:8000") as client:
        ...
        logger.info("Test created successfully in %.2f seconds:", test_creation_time)
        ...
        if test.test_status == Status.COMPLETED:
            ...
            if score_run_response.score_run_status == Status.COMPLETED:
                logger.info(
                    "Score run Complete in %.2f seconds: %s",
                    scoring_time,
                    score_run_response.score_run_uuid,
                )
                logger.info(
                    "Score run Status: %s", score_run_response.score_run_status.value
                )
                logger.info(
                    "Number of scored answers: %s", len(score_run_response.answers)
                )
