...
class EvalMixin(AymaraAIProtocol):
    def create_instruct_eval(
        self,
        eval_name: str,
        student_description: str,
        eval_policy: str,
        eval_language: str = DEFAULT_TEST_LANGUAGE,
        ...
        return self._create_eval(
            ...
            test_policy=eval_policy,
            test_system_prompt=None,
            knowledge_base=None,
            test_language=eval_language,
    ...
    create_instruct_eval.__doc__ = f"""
    ...
    async def create_instruct_eval_async(
        self,
        test_name: str,
        student_description: str,
        test_policy: str,
    ...
    create_instruct_eval_async.__doc__ = f"""
    ...
    def _create_eval(
        ...
        test_data = models.EvalInSchema(
            eval_name=eval_name,
            num_prompts=num_test_questions or DEFAULT_NUM_QUESTIONS,
            eval_template_key=test_type,
            eval_config=models.EvalInSchemaEvalConfig.from_dict({
                "student_description": student_description,
                "test_policy": test_policy,
                "test_system_prompt": test_system_prompt,
                "knowledge_base": knowledge_base,
        ...
        if is_async:
            return self._create_and_wait_for_eval_impl_async(
                test_data, max_wait_time_secs, is_sandbox
            )
        else:
            return self._create_and_wait_for_eval_impl_sync(
                test_data, max_wait_time_secs, is_sandbox
            )
    ...
    def _validate_eval_inputs(
        ...
        if (
            len(eval_name) < DEFAULT_TEST_NAME_LEN_MIN
            or len(eval_name) > DEFAULT_TEST_NAME_LEN_MAX
        ):
            raise ValueError(
    ...
    def _create_and_wait_for_eval_impl_sync(
        self,
        test_data: models.EvalInSchema,
        max_wait_time_secs: Optional[int],
        ...
        response = create_eval.sync_detailed(
    ...
    async def _create_and_wait_for_eval_impl_async(
        self,
        test_data: models.EvalInSchema,
        max_wait_time_secs: Optional[int],
        is_sandbox: Optional[bool] = None,
        ...
        start_time = time.time()
        response = await create_eval.asyncio_detailed(
            client=self.client, body=test_data
        )
    ...
    # Get Test Methods
    def get_eval(self, test_uuid: str) -> BaseTestResponse:
        """
        ...
        :rtype: TestResponse
        """
        return self._get_eval(test_uuid, is_async=False)

    async def get_eval_async(self, test_uuid: str) -> BaseTestResponse:
        """
        ...
        :rtype: TestResponse
        """
        return await self._get_eval(test_uuid, is_async=True)
    ...
    def _get_eval(
        ...
        if is_async:
            ...
            return self._get_eval_sync_impl(test_uuid)

    def _get_eval_sync_impl(self, test_uuid: str) -> BaseTestResponse:
    ...
    # List Tests Methods
    def list_evals(self) -> ListTestResponse:
        """
        List all tests synchronously.
        """
        tests = self._list_evals_sync_impl()

        return ListTestResponse(tests)

    async def list_evals_async(self) -> ListTestResponse:
        """
        List all tests asynchronously.
        """
        tests = await self._list_evals_async_impl()

        return ListTestResponse(tests)

    def _list_evals_sync_impl(self) -> List[BaseTestResponse]:
        all_tests = []
        offset = 0
        while True:
    ...
    async def _list_evals_async_impl(self) -> List[BaseTestResponse]:
        all_tests = []
        offset = 0
        while True:
    ...
    # Helper Methods
    def _get_all_prompts_sync(self, test_uuid: str) -> List[models.QuestionSchema]:
        questions = []
        offset = 0
        while True:
            response = get_eval_questions.sync_detailed(
    ...
    async def _get_all_prompts_async(
        self, test_uuid: str
    ) -> List[models.QuestionSchema]:
        questions = []
        offset = 0
        while True:
            response = await get_eval_questions.asyncio_detailed(
                client=self.client, test_uuid=test_uuid, offset=offset
            )
